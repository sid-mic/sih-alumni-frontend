...
export default function AdminChangeMaker(props) {
  ...
  if (!data) {
    ...
    return (
      <div className="text-center mt-10">
        <h1 className={"font-bold"}>No Videos yet!</h1>
      </div>
    );
  } else {
    return (
...
function ChangeMakerCard({ data, editform, setpage, setlist }) {
  function handleDelete(id) {
    toast.promise(axios().post(`changemaker/${id}`, { _method: "delete" }), {
      pending: {
        render() {
          return "Deleting.....";
        },
