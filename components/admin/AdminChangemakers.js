...
export default function AdminChangemakers(props) {
  const [page, setpage] = useState(0);
  const [editpage, seteditform] = useState({});

  const [changemaker_list_data, setlistdata] = useState(false);
  const [meta, setMeta] = useState({});

  useEffect(() => {
    if (!changemaker_list_data) {
      handleRequest();
    }
  }, [props.user, changemaker_list_data]);

  function handleRequest(page = 1) {
    setlistdata(false);
    axios()
      .get(`changemakers?page=${page}`)
  ...
  if (page === 0) {
    ...
    return (
      <>
        ...
        <AdminChangemakerList
          data={changemaker_list_data}
  ...
  if (page === 1) {
    return (
      <AdminChangemakerForm
        setpage={setpage}
        list={changemaker_list_data}
        setlist={setlistdata}
      />
    );
  }
  ...
  if (!data) {
    ...
    return (
      <div className="text-center mt-10">
        <h1 className={"font-bold"}>No changemakers yet!</h1>
      </div>
    );
  } else {
    return (
...
function ChangemakerCard({ data, editform, setpage, setlist }) {
  function handleDelete(id) {
    toast.promise(axios().post(`changemakers/${id}`, { _method: "delete" }), {
      pending: {
        render() {
          return "Deleting.....";
        },
...
function AdminChangemakerForm(props) {
  let data = props.data;
    ...
    toast.promise(axios().post(post_url, form_data), {
      ...
      success: {
        render() {
          ...
          setlist(false);
          return data?.id
            ? "Changemaker Video updated successfully!"
            : "Changemaker Video added successfully!";
        },
      },
      error: {
        render({ data }) {
    ...
    <div className=" min-h-screen  ml-5 lg:ml-20 mr-5 lg:mr-20">
      ...
      <div className="flex -mx-3">
        <div className="w-full px-3 mb-5">
          ...
          >
            {data?.id ? "UPDATE VIDEO" : "ADD VIDEO"}
          </button>
        </div>
      </div>
    </div>
